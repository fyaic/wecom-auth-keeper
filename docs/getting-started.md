...
在目标 bot 详情打开“可使用权限”，将需要续期的行滚动到可见位置：
...
仅处理指定能力中 24 小时内到期的行；已到期的行直接重授，健康且远于阈值的行跳过。每次只取消并恢复一条，再进入下一条。此命令不会自动导航管理列表或滚动不可见控件；相关自动导航仍待实现。
...
此独立状态机已通过模拟故障回归，但尚未进行新版代码的真实重授权验收。9 月 21 日的实机记录属于桌面代理操作，不能代替这项验收。
